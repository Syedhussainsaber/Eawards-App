...
try{
 ...
 const movieRes = await fetch(`https://eawards.vercel.app/api/movieDetails/${popularMovies?.results[0]?.id}`)
   const movieDetails = await movieRes.json()

const castsRes = await fetch(`https://eawards.vercel.app/api/movieCastings/${popularMovies?.results[0]?.id}`)
